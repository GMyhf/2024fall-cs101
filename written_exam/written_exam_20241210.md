...
Updated 0127 GMT+8 Dec 09, 2024
...
冯·诺依曼“存储程序” 运算器、存储器、控制器、I/O设备。CPU包括运算器、控制器。

n位能表示 0~2^n-1

补码：负数按位求反+1 或 [2^n+x]原 -2^n~2^n-1

符号位S+11位e+52位f 	双精度数 s×1.p×2^±e

e: -1022~1023

> 计算机科学中浮点数的表示方法，特别是IEEE 754标准下的双精度浮点数（64位）。
>
> 1. **补码：负数按位求反+1 或 [2^n+x]     原 -2^n~2^n-1**
>
>    这部分描述的是二进制补码的概念。在计算机系统中，整数通常以补码形式存储。对于一个n位的二进制数：
>    
>    - 正数的补码就是其本身。
>    - 负数的补码是该数绝对值的二进制表示逐位取反（即0变1，1变0），然后加1。
>    - 另一种表达方式是，如果x是一个负数，它的补码可以被看作是 \(2^n + x\)，其中\(2^n\)是下一个更高的2的幂次。
>
> 2. **符号位S+11位e+52位f       双精度数 s×1.p×2^±e**
>
>    这里描述的是IEEE 754标准下双精度浮点数的格式：
>    
>    - 符号位（S）：1位，用于表示数字的正负，0代表正数，1代表负数。
>    - 指数位（e）：11位，用来表示指数，采用偏置表示法，实际的指数值等于存储的值减去一个偏置量（对于双精度，偏置量为1023）。
>    - 尾数/小数位（f）：52位，表示有效数字的小数部分。实际上，尾数是1加上这52位所代表的二进制分数。
>
>    因此，一个双精度浮点数可以用公式 $ (-1)^s \times (1 + f) \times 2^{(e-1023)} $ 来表示，其中\(s\)是符号位，\(f\)是尾数，\(e\)是指数。
>
> 3. **e: -1022~1023**
>
>    这个范围指的是双精度浮点数的指数部分的有效范围。指数的实际值是从偏置后的值计算出来的。由于指数字段有11位，它可以表示从0到2047的值。但是，为了表示负指数，使用了偏置1023，因此有效的指数范围是-1022到1023。特殊情况下，全0和全1的指数值保留给特殊数值如零、无穷大和非数值（NaN）。
>
> 综上所述，这些描述总结了如何在计算机中表示负数（通过补码），以及如何根据IEEE 754标准表示双精度浮点数，包括它们的符号、指数和尾数。

加：000000001 -> 减 -> 乘 -> 除 -> 跳转（...101）

ASCII：8位，首位为0（校验位），表示2^7种
...
声音的波形编码：时间 & 波形高度，MIDI

颜色：n位就有2^n种

汉字的输入码：数字编码（2位码：国际码 -00100000）
...
1. 根据<mark>局部性</mark>原理，在程序执行时，如果一个信息项正在被访问，那么近期它很可能也会被再次访问，存储在它附近的信息也很可能被访问到。

2. 表达式 `(((A + 1) < 50) || (5 * ('a' > 90))) + 1` 的值是<mark>2</mark>，类型是<mark>int</mark>。（字符 A 的 ASCII 码数值是 65）

3. 在登录 12306 网站购票时，除了要求输入用户名、密码之外，还需要购票者按要求点击网页上的图片，这实际上就是要用户输入验证码，它的作用是比较好的解决了对<mark>密码</mark>的暴力破解问题。
...
10. 对于按如下定义的数组 `int a[10] = {-1, 2, 5, 1, 8};`，`a[7] - a[4]` 的值是 <mark>-8</mark>。

> 在C语言中，数组的索引是从0开始的。这意味着对于一个声明为`int a[10];`的数组，有效的索引范围是0到9。然而，在您的代码中尝试访问`a[7]`，这实际上是合法的，因为它处于这个范围内。
>
> 给定的数组初始化如下：
>
> ```c
...
> ```
>
> 这里只显式地初始化了前5个元素，即`a[0]`到`a[4]`。剩下的元素（`a[5]`到`a[9]`）将被自动初始化为0，这是因为当使用大括号`{}`进行初始化时，未指定初始值的元素会被默认初始化为0。
>
> 所以，根据上述规则：
>
> - `a[4]` 的值是 8（这是您提供的初始化列表中的最后一个值）。
> - `a[7]` 的值是 0（因为它是未显式初始化的元素之一，默认为0）。
>
...
4. 判断字符变量 c 的值不是数字也不是字母时，应采用下述哪个表达式。注：关系运算的优先级高于逻辑运算。
   ...
   <mark>C)</mark> !((c >= '0' && c <= '9') || !(c >= 'A' && c <= 'Z') || !(c >= 'a' && c <= 'z'))
...
​	<mark>C)</mark> 变量的值存储于如硬盘、U 盘等外存之中
...
1. 二进制逻辑运算（4分，与：&，或：|，非：~，异或：^）
   - 1010 1001 & (~ 0101 0011) = (<mark>10101000</mark>)
...
3. 视频编码中“帧率”是指单位时间内包含的图像帧数，一般使用 FPS（帧/秒）为单位。50 FPS 即每秒钟播放 50 帧图像。常见的 1080P 原始视频每帧图像包含 1920×1080 个像素点，每个像素点使用 8 位进行存储。（计算时 1920×1080 可以算作 2,000,000；1 GB = 1000 MB，其余进制同理）
   ...
   (1) 一段 5 分钟的 1080P 50FPS 的原始视频约占多少 GB 的存储空间？（结果仅保留整数）（ 3 分）
   	<mark>`2000*1000*50*300*8/1000/1000/1000/8 = 30 GB`</mark>
   ...
   （结果仅保留整数）（ 1 分）
   	`128*1000*1000*1000*600/2000/1000/50/3600 = 213 小时`
...
```c
// fetchIPAddress() 是一个自定义函数，用来输入一个点分制表示的 IP 地址，
//并将其转换成二进制 IP 地址
unsigned int fetchIPAddress()
{
    int a, b, c, d;
    ...
    // 10010111>>2， 表示二进制数右移 2 位得到 00100101。 
    // “|” 是二进制数按位操作
...
int main()
{
    ...
    int blacklistsize;
    scanf("%d", &blacklistsize);
    unsigned int blacklist[1000];
    for (int i = 0; i < blacklistsize; i++)
        blacklist[i] = fetchIPAddress();

    // 读入发送地址和接收地址
    unsigned int senderIP = fetchIPAddress();
    unsigned int receiverIP = fetchIPAddress();
    ...
    for (int i = 0; i < blacklistsize; i++) // 【3】
    {
        if (senderIP == blacklist[i])
        {
            // 屏蔽
            break;
        }
...
A. scanf("%d.%d.%d.%d", &a, &b, &c, &d);

B. scanf("%d%d%d%d", &a, &b, &c, &d);

C. scanf("%d %d %d %d", &a, &b, &c, &d);
...
unsigned int subnetID = (receiverIP & <mark>0x3fff0000 或 二进制表示</mark> ______) >> <mark>16</mark>;
...
int main() {
    ...
    switch (op) {       // 【2】
        ...
        case '/':
            if (b == 0) {       // 【3】
                printf("The variable b can NOT be zero!\n");
            } else {
                printf("%d / %d = %d\n", a, b, a / b);  // 【4】
            }
            break;
...
提示：
1：语句 scanf("%s",s); 表示读入字符串 s， s 是一个字符数组
2: 函数 strlen(s) 返回值为字符串 s 的长度
...
int main() {
    ...
    scanf("%s", s); // 以字符串的形式读入这个正整数，如输入 "12345" 时，数组 s 中的每个元素是一个数字字符，s[0] 为 '1'，s[2] 为 '3'，...
    int n = strlen(s); // s 表示的这个正整数共有 n 位
    for (int i = 0; i < n; ++i) {
        if (s[i] < '0' || s[i] > '9') {		// 【1】
    ...
    if (feature_11 % 11 == 0 && a[n - 1] % 2 == 0) {	// 【4】【5】
        printf("YES\n");
    } else {
        printf("NO\n");
    }
